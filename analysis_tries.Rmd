...
tweets1 <- tweets %>%
  ...
  mutate(date = date(strptime(timestamp, "%Y/%m/%d %H:%M:%S"))) %>% as.character()
  filter(date > "2020-01-23" & date < "2020-01-30") %>%
  unnest_tokens(word, text) %>%
  anti_join(custom_stop_words) %>%
  count(word,  sort=T) %>% #56851
  filter(n > 10) 
...
tweets %>%
  #select(timestamp, text) %>%
  #mutate(date = date(strptime(timestamp, "%m/%d/%Y %H:%M:%S"))) %>% as.character
```
